...
def view_users(request):
    ...
    query = """
        SELECT CONVERT(date, char), COUNT(BDonationID)
        FROM blooddonation
        WHERE Date >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)
        GROUP BY DATE(Date) ORDER BY Date DESC;
    """
    cur.execute(query)
    BloodRecords = cur.fetchall()
    
    query = """
        SELECT CONVERT(SurgeryDate, char), COUNT(TransplantationID)
...
def view_donor_details(request):
    ...
    print(query, " :    : ", OrganDonationRecords)
