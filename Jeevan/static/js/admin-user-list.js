...
const userStats = document.querySelector(".user-count-stats")

const hospitalCount = userStats.dataset.hospital
const donorCount = userStats.dataset.donor
const patientCount = userStats.dataset.patient
...
for (let i = 6; i >= 0; i--) {
    ...
    lastSevenDays.push(bloodFormattedDate);
}
...
const bloodFinalData = lastSevenDays.map(date => {
...
const organFinalData = lastSevenDays.map(date => {
